...
def test_web_login_serves_javascript_asset() -> None:
    ...
    assert response.status_code == 200
    assert "async function handleLogin" in response.text
    assert "/api/v1/auth/login" in response.text
    assert "area51_access_token" in response.text
...
def test_web_map_returns_404_when_map_was_not_generated() -> None:
