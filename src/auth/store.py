...
class BaseStore(ABC, Generic[T]):
    ...
    def _remove_where(self, predicate: Any) -> int:
        with self._lock:
            keys = [k for k, v in self._data.items() if predicate(v)]
            for k in keys:
                del self._data[k]
            if keys:
                self._save()
    ...
    def _filter(self, predicate: Any) -> list[T]:
        return [v for v in self._all_items() if predicate(v)]
...
class ApiKeyJsonStore(BaseStore["ApiKey"]):
    ...
    def active(self) -> list["ApiKey"]:
        return self._filter(lambda k: k.is_usable)
    ...
    def remove_revoked(self) -> int:
        ...
        return self._remove_where(lambda k: k.status == KeyStatus.REVOKED)
...
class SessionJsonStore(BaseStore[dict[str, Any]]):
    ...
    def get_active(self) -> list[dict[str, Any]]:
        return self._filter(lambda s: s.get("active", False))

    def remove(self, session_id: str) -> bool:
        return self._remove(session_id)

    def remove_inactive(self) -> int:
        return self._remove_where(lambda s: not s.get("active", False))
...
class UserJsonStore(BaseStore["User"]):
    ...
    def get_by_email(self, email: str) -> "User | None":
        results = self._filter(lambda u: u.email == email)
